...
extern "C"
    ...
    __global__ void count_problems_gpu(ulong *choose_table, ushort *ramsey, ulong *num_cliques_cum, unsigned short *edge_idx_to_pair, unsigned short *edge_pair_to_idx, ushort *clique_list, unsigned short *edge_list, ushort *colorings, ushort *problems, uint *problem_counts)
    {        
        uint block_idx = blockIdx.x + blockIdx.y*gridDim.x + blockIdx.z*gridDim.x*gridDim.y;
        uint thread_idx_loc = threadIdx.x + threadIdx.y*blockDim.x + threadIdx.z*blockDim.x*blockDim.y;
        //uint thread_idx_glob = block_idx*blockDim.x*blockDim.y*blockDim.z + thread_idx_loc;
