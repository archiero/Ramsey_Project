...
choose_table = np.array([[choose(n,k) for n in range(num_vertices+1)] for k in range(biggus_clickus+1)]).astype('uint64')
#NOTE: choose_table[k,n] gives binom(n,k).  This is strange, but works better later with the combinatorial numbering system.
tot_edges = int(choose_table[2,num_vertices])

num_edges_per_clique = np.array([choose_table[2,ram] for ram in ramsey])

num_cliques = np.array([choose_table[ram,num_vertices] for ram in ramsey])
...
extern "C"
    ...
    __device__ void get_assignment(unsigned long *choose_table, unsigned char *ramsey, unsigned long *num_cliques_cum, unsigned short *edge_idx_to_pair, unsigned short *edge_pair_to_idx, unsigned char *color, unsigned long *clique_idx, unsigned char *n_verts, unsigned char *verts, unsigned short *n_edges, unsigned short *edges)
    {
    ...
    __global__ void count_problems_gpu(unsigned long *choose_table, unsigned char *ramsey, unsigned long *num_cliques_cum, unsigned short *edge_idx_to_pair, unsigned short *edge_pair_to_idx, unsigned char *clique_list, unsigned short *edge_list, unsigned char *colorings, unsigned char *problems, unsigned int *problem_counts)
    {
        ...
        if(block_idx < TOT_CLIQUES)
        {
            unsigned int i, start;
            __shared__ unsigned char color, n_verts;
            __shared__ unsigned short n_edges;
            __shared__ unsigned long clique_idx;

            __shared__ unsigned char verts[MAX_VERTS];
...
regex_fixing_dictionary = {"TOT_VERTS":num_vertices, "TOT_EDGES":tot_edges, "TOT_CLIQUES": tot_cliques
