...
def write_coloring(coloring, problems, file):
    app_coloring = "".join(["data.append(", str(coloring).replace(" ", ","),")"])
    app_target = "".join(["target.append(", str(problems), ")"])
    append_to_file([app_coloring,app_target], file)
...
def unit(v,axis=0):
    ...
    v /= m
    return v
#This is several functions that back up and reload the neural network appropiately
